#!/c/pythonaqui/python
# para executar sysargs é necessario o caminho do interpretador do python
import sys
import csv
import os
...
def process_csv(csv_file):
    ...
    with open(csv_file,"r") as datafile:
        data = list(csv.reader(datafile)) # csv.reader() vai retornar um objeto iteravel contendo uma lista de listas que contem as linhas do csv
    return data                           # list() vai converter o objeto para uma lista de listas

def data_to_html(title, data):
    """Turns a list of lists into an HTML table"""

    # HTML Headers , aspas triplas servem para strings de mais de uma linha
    html_content = """
<html>
<head>
<style>
...
"""
    ...
    for i, row in enumerate(data): # enumerate vai pegar um objeto iteravel e devolver uma sequencia de tuplas com index e elementos
        html_content += "<tr>" # <tr>(table row) tag para linha dentro da tabela
        for column in row:
            if i == 0: # caso o primeiro elemento vai adicionar como header da tabela
                html_content += "<th>{}</th>".format(column) # <th>(table header)tag para header dentro da tabela
            else:
                html_content += "<td>{}</td>".format(column) # <td>(table data) tag para data dentro da tabela
        html_content += "</tr>"

    html_content += """</tr></table></body></html>""" # tags de fechamento
    return html_content
